...
def get_data_from_FG_API_with_start_end(variableid, start_time, end_time):
    headers = {'x-api-key': os.environ['FGAPIKEY']}
    params = {'start_time': start_time, 'end_time': end_time, 'response_timezone': "+03:00"}
    print(start_time, end_time)
    r = requests.get(f'http://api.fingrid.fi/v1/variable/{variableid}/events/csv', params=params, headers=headers,
                     verify=False)
    content = r.content.decode('utf-8')
